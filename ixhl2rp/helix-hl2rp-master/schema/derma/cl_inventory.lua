...
function PANEL:BuildSlots()
	...
	local function PaintSlot(slot, w, h)
		...
		surface.SetDrawColor(OC_Config.MainColor.r, OC_Config.MainColor.g, OC_Config.MainColor.b, 100)
		surface.DrawRect(1, 1, w - 2, h - 2)
	end
...
hook.Add("CreateMenuButtons", "ixInventory", function(tabs)
	...
	tabs["inv"] = {
		...
		Create = function(info, container)
			...
			panel:SetTitle("Character Inventory")
			...
			local iconpanel = canvas:Add("DFrame")
			iconpanel:SetPos(0, 200)
			iconpanel:SetSize(300, 600)
			iconpanel:SetDraggable(false)
			iconpanel:SetSizable(false)
			iconpanel:ShowCloseButton(true)
			iconpanel:SetTitle("Character Display")
			
			local ModelRotSlider = vgui.Create( "DNumSlider", iconpanel )
			ModelRotSlider:SetPos(-180, 30)
			ModelRotSlider:SetSize(480, 20)
			ModelRotSlider:SetMin(0)
			ModelRotSlider:SetMax(360)
			ModelRotSlider:SetDecimals(0)
			
			local modelPreview = vgui.Create("ixModelPanel", iconpanel)
			modelPreview:SetPos(-30, 0)
			modelPreview:Dock(FILL)
			modelPreview:SetModel(LocalPlayer():GetModel(), LocalPlayer():GetSkin(), LocalPlayer():GetBodyGroups())
			...
			function modelPreview:LayoutEntity( ent )
				...
				entity:SetAngles( Angle( 0, ModelRotSlider:GetValue() + 40, 0 ) )
				entity:SetIK(false)
			...
			local info = canvas:Add("DFrame")
			info:SetPos(0, 200)
			info:SetSize(500, 200)
			info:SetDraggable(true)
			info:SetSizable(false)
			info:ShowCloseButton(true)
			info:SetTitle("Character Information")
			local format = "%A, %B %d, %Y. %H:%M"

			infotime = info:Add("DLabel")
			infotime:SetFont("ixMediumFont")
			infotime:SetContentAlignment(1)
			infotime:Dock(TOP)
			infotime:SetTextColor(color_white)
			infotime:SetExpensiveShadow(1, Color(0, 0, 0, 150))
			infotime:DockMargin(4, 0, 0, 8)
			infotime:SetText(ix.date.GetFormatted(format))
			infotime.Think = function(this)
				if ((this.nextTime or 0) < CurTime()) then
					this:SetText("IC Date : "..ix.date.GetFormatted(format))
					this.nextTime = CurTime() + 4
				end
			end

			infofaction = info:Add("DLabel")
			infofaction:SetFont("ixMediumFont")
			infofaction:SetContentAlignment(1)
			infofaction:Dock(TOP)
			infofaction:SetTextColor(color_white)
			infofaction:SetExpensiveShadow(1, Color(0, 0, 0, 150))
			infofaction:DockMargin(4, 0, 0, 8)
			infofaction:SetText(ix.date.GetFormatted(format))
			infofaction.Think = function(self)
				if ((self.nextTime or 0) < CurTime()) then
					self:SetText("Faction : "..team.GetName(LocalPlayer():Team()))
					self.nextTime = CurTime() + 4
				end
			end
			
			local class = ix.class.list[LocalPlayer():GetCharacter():GetClass()]

			infoclass = info:Add("DLabel")
			infoclass:SetFont("ixMediumFont")
			infoclass:SetContentAlignment(1)
			infoclass:Dock(TOP)
			infoclass:SetTextColor(color_white)
			infoclass:SetExpensiveShadow(1, Color(0, 0, 0, 150))
			infoclass:DockMargin(4, 0, 0, 8)
			infoclass:SetText(ix.date.GetFormatted(format))
			infoclass.Think = function(self)
				if ((self.nextTime or 0) < CurTime()) then
					self:SetText("Class : "..class.name or "INVALID")
					self.nextTime = CurTime() + 4
				end
			end
		end
